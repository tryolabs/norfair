# This file is adapted from the original FilterPy library by Roger R Labbe Jr.
...
This version has been modified for integration into the Norfair tracking framework.
...
class KalmanFilter:
    ...
    **Example**
    ...
    def get_update(
        ...
        # Precompute common subexpression for efficiency
        PHT = P @ H.T
    ...
    def test_matrix_dimensions(
        ...
        assert Q.shape == (
            self.dim_x,
            self.dim_x,
        ), f"Shape of Q must be ({self.dim_x}, {self.dim_x}), but is {Q.shape}"
        assert F.shape == (
            self.dim_x,
            self.dim_x,
        ...
        if z_shape == ():  # scalar or np.array(scalar)
            ...
            assert z_shape[0] == 1, f"Shape of z must be {np.shape(Hx)} for the given H"
        else:
...
def update(
    ...
    This function handles both multidimensional and unidimensional cases.
    If all parameters are floats instead of arrays, the filter will still work and
    return floats for x and P as the result.
    ...
    P : np.ndarray or float
        Covariance matrix.
    z : array_like (dim_z, 1) or scalar
        Measurement for this update. z can be a scalar if dim_z is 1,
        otherwise it must be convertible to a column vector.
    R : np.ndarray or float
        Measurement noise matrix.
    H : np.ndarray, float, or tuple, optional
        Measurement function. If not provided, a value of 1 is assumed.
    return_all : bool, default False
        If True, returns (x, P, y, K, S, log_likelihood), otherwise only (x, P).
    ...
    y : np.ndarray or scalar
        Residual between measurement and state in measurement space.
    K : np.ndarray
        Kalman gain.
    S : np.ndarray
